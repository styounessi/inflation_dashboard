...
fig2.add_scattergeo(locations=gas['State'],
                    locationmode='USA-states',
                    text=gas['State'],
                    mode='text',
                    hoverinfo='skip')
...
hpi['% Change'] = hpi['CSUSHPINSA'].pct_change().round(4) * 100
cpi['% Change'] = cpi['CPIAUCSL'].pct_change().round(4) * 100
ppi['% Change'] = ppi['PPIACO'].pct_change().round(4) * 100
pce['% Change'] = pce['PCEPI'].pct_change().round(4) * 100
...
app.layout = dbc.Container([
    ...
    dbc.Row([
        dbc.Col([
            dcc.Dropdown(id='drpdwn', multi=False, value='Crude Oil',
